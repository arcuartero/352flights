...
export function validateCreatelloDocument(document:Record<string,unknown>){
  ...
  for(const item of offers){if(!item||typeof item!=="object")throw new Error("Hay una oferta no válida."); const offer=item as Record<string,unknown>; const price=offer.price; if(typeof price!=="number"||!Number.isFinite(price)||price<=0)throw new Error("Todos los precios deben ser números válidos."); for(const field of ["origin","destination","originAirport","destinationAirport","airport"]){const value=offer[field]; if(typeof value==="string"&&(value!==value.toUpperCase()||!/^[A-Z]{3}$/.test(value)))throw new Error(`${field} debe ser un código IATA en mayúsculas.`);} const key=[offer.origin??offer.originAirport,offer.destination??offer.destinationAirport??offer.airport,offer.outboundDate??offer.departure,offer.returnDate].join(":"); if(seen.has(key))throw new Error("El JSON contiene ofertas duplicadas."); seen.add(key); const destinationKey=keyText(String(offer.title??offer.destination??offer.airport??offer.destinationAirport)); if(seenDestinations.has(destinationKey))throw new Error("El JSON contiene destinos repetidos."); seenDestinations.add(destinationKey);}
}
