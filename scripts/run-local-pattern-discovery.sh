...
if [[ -f "$ROOT_DIR/.env" ]]; then
  while IFS='=' read -r key value; do
    case "$key" in
      SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SCANNER_CURRENCY|SCANNER_HISTORY_WINDOW|SCANNER_REVIEW_RATIO|SCANNER_FLASH_RATIO|SCANNER_WEEKEND_PATTERN_DISCOVERY_END_DAYS|SCANNER_LONG_HAUL_PATTERN_DISCOVERY_END_DAYS|SCANNER_PATTERN_OVERRIDE_VALID_DAYS)
        export "$key=$value"
        ;;
    esac
  done < <(grep -E '^(SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SCANNER_CURRENCY|SCANNER_HISTORY_WINDOW|SCANNER_REVIEW_RATIO|SCANNER_FLASH_RATIO|SCANNER_WEEKEND_PATTERN_DISCOVERY_END_DAYS|SCANNER_LONG_HAUL_PATTERN_DISCOVERY_END_DAYS|SCANNER_PATTERN_OVERRIDE_VALID_DAYS)=' "$ROOT_DIR/.env")
fi
