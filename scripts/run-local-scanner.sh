...
if [[ -f "$ROOT_DIR/.env" ]]; then
  while IFS='=' read -r key value; do
    case "$key" in
      SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SCANNER_CURRENCY|SCANNER_HISTORY_WINDOW|SCANNER_MIN_HISTORY_FOR_DEAL|SCANNER_REVIEW_RATIO|SCANNER_BOOTSTRAP_VISIBLE_DEALS_PER_DESTINATION|SCANNER_BOOTSTRAP_REVIEW_RATIO|SCANNER_FLASH_RATIO)
        export "$key=$value"
        ;;
    esac
  done < <(grep -E '^(SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SCANNER_CURRENCY|SCANNER_HISTORY_WINDOW|SCANNER_MIN_HISTORY_FOR_DEAL|SCANNER_REVIEW_RATIO|SCANNER_BOOTSTRAP_VISIBLE_DEALS_PER_DESTINATION|SCANNER_BOOTSTRAP_REVIEW_RATIO|SCANNER_FLASH_RATIO)=' "$ROOT_DIR/.env")
fi
