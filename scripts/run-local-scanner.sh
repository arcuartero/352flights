...
if [[ -f "$ROOT_DIR/.env" ]]; then
  while IFS='=' read -r key value; do
    case "$key" in
      SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SCANNER_CURRENCY|SCANNER_HISTORY_WINDOW|SCANNER_MIN_HISTORY_FOR_DEAL|SCANNER_REVIEW_RATIO|SCANNER_FLASH_RATIO|SCANNER_SYNC_SNAPSHOTS_LIVE|SCANNER_SYNC_DEALS_LIVE|SCANNER_PROVIDER_PREFLIGHT_ENABLED|SCANNER_PROVIDER_PREFLIGHT_DESTINATIONS|SCANNER_PROVIDER_PREFLIGHT_DAYS_AHEAD|SCANNER_PROVIDER_ERROR_ATTEMPTS|SCANNER_PROVIDER_ERROR_BASE_SECONDS|SCANNER_PROVIDER_ERROR_MAX_SECONDS|SCANNER_EMPTY_RESULT_BREAKER_THRESHOLD)
        export "$key=$value"
        ;;
    esac
  done < <(grep -E '^(SUPABASE_URL|SUPABASE_SERVICE_ROLE_KEY|SCANNER_CURRENCY|SCANNER_HISTORY_WINDOW|SCANNER_MIN_HISTORY_FOR_DEAL|SCANNER_REVIEW_RATIO|SCANNER_FLASH_RATIO|SCANNER_SYNC_SNAPSHOTS_LIVE|SCANNER_SYNC_DEALS_LIVE|SCANNER_PROVIDER_PREFLIGHT_ENABLED|SCANNER_PROVIDER_PREFLIGHT_DESTINATIONS|SCANNER_PROVIDER_PREFLIGHT_DAYS_AHEAD|SCANNER_PROVIDER_ERROR_ATTEMPTS|SCANNER_PROVIDER_ERROR_BASE_SECONDS|SCANNER_PROVIDER_ERROR_MAX_SECONDS|SCANNER_EMPTY_RESULT_BREAKER_THRESHOLD)=' "$ROOT_DIR/.env")
fi
...
elif (( EXIT_CODE == 69 )); then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Local Lux flight scan stopped because the provider is unavailable." >> "$STDOUT_LOG"
